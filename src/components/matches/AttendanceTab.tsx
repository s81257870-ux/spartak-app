...
import { UserCheck, X, CheckCircle2, ChevronDown } from 'lucide-react'
...
export default function AttendanceTab({ matchId }: Props) {
  ...
  // Who this device's user is — persisted across visits
  const [myPlayerId, setMyPlayerId] = useState<string>(
    () => localStorage.getItem(MY_PLAYER_KEY) ?? ''
  )
  // Whether to show the player picker (true when no player saved yet, or user clicked "Skift")
  ...
  // Map attendance IDs → player objects for the list
  ...
  const handleSignUp = () => {
    if (!myPlayerId || isSignedUp) return
    signUp(matchId, myPlayerId)
  }

  const handleCancel = () => {
    cancelSignUp(matchId, myPlayerId)
  }
    ...
    <div className="space-y-5">
      ...
      <div className="bg-[#1a1d27] rounded-2xl p-4 space-y-4">
        ...
        {!showPicker && myPlayerId !== '' && (
          isSignedUp ? (
            /* Already signed up → success state */
            ...
            /* Not yet signed up → signup button */
            <button
            ...
            {signedUpEntries.map(({ id, player }) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Full name */}
                  <span className={`flex-1 text-sm font-medium ${isMe ? 'text-orange-300' : 'text-white'}`}>
                    {fullName}
                    {isMe && (
                      <span className="ml-1.5 text-orange-500/70 text-xs font-normal">(dig)</span>
                    )}
                  </span>
                  ...
                  {/* Cancel — visible for own row, or always (admin can also cancel) */}
                  <button
